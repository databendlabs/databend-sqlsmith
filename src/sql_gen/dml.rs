...
use crate::catalog::schema::TableField;
use crate::catalog::types::DataType;
use crate::catalog::types::resolve_type_name;
...
impl<'a, R: Rng + 'a> SqlGenerator<'a, R> {
    ...
    fn gen_update(&mut self) -> UpdateStmt {
        ...
        for field in fields {
            update_list.push(MutationUpdateExpr {
                ...
                expr: self.gen_scalar_value(field.data_type()),
            });
        }
        UpdateStmt {
            hints,
        ...
        for i in 0..opt_num {
            ...
            let merge_opt = if self.rng.gen_bool(0.5) {
                let operation = if self.rng.gen_bool(0.7) {
                    ...
                    for field in fields {
                        ...
                        let update_expr = MutationUpdateExpr {
                            table: None,
                            name: Identifier::from_name(None, field.name()),
                            expr: self.gen_expr(field.data_type()),
                        };
                ...
                let (columns, values) = if self.rng.gen_bool(0.5) {
                    ...
                    let values = fields
                        .iter()
                        .map(|f| self.gen_scalar_value(f.data_type()))
                        .collect::<Vec<_>>();
        ...
        match self.rng.gen_range(0..=9) {
            0..=9 => {
                ...
                for i in 0..row_count {
                    ...
                    raw_values.push('(');
                    for (j, data_type) in data_types.iter().enumerate() {
                        if j > 0 {
                            raw_values.push(',');
                        }
                        raw_values.push_str(&self.gen_insert_value(data_type));
                    }
                    raw_values.push(')');
                }
    ...
    fn gen_insert_value(&mut self, data_type: &DataType) -> String {
        self.gen_scalar_value(data_type).to_string()
    }
}
