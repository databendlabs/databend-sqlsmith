...
use crate::catalog::types::convert_to_type_name;
...
impl<R: Rng> SqlGenerator<'_, R> {
    ...
    fn gen_column(&mut self, ty: &DataType) -> Expr {
        for bound_column in &self.bound_columns {
            if data_type_eq(&bound_column.data_type, ty) {
    ...
    fn gen_literal(&mut self, ty: &DataType) -> Literal {
        match ty {
            ...
            DataType::Decimal(decimal) => Literal::Decimal256 {
                ...
                precision: decimal.decimal_size().precision,
                scale: decimal.decimal_size().scale,
            },
            _ => Literal::Null,
        }
    }
    ...
    pub(crate) fn gen_points(&mut self) -> String {
        ...
        for _ in 0..self.rng.gen_range(1..=6) {
            let point = self.gen_point().to_string();
            points.push(point);
        }
        points.join(", ")
    }
