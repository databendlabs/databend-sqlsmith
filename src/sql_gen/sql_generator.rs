...
use crate::catalog::functions::FunctionSignature;
use crate::catalog::functions::builtin_scalar_function_signatures;
...
impl<'a, R: Rng> SqlGenerator<'a, R> {
    pub(crate) fn new(rng: &'a mut R, settings: Vec<(String, DataType)>) -> Self {
        SqlGenerator {
            rng,
            settings,
            scalar_func_sigs: builtin_scalar_function_signatures(),
            tables: vec![],
            cte_tables: vec![],
            bound_tables: vec![],
            bound_columns: vec![],
